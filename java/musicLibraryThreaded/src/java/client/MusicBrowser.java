...
public class MusicBrowser extends MediaLibraryGui implements
   ...
   private MusicBrowserTcpProxy mediaConnection;
   ...
   public MusicBrowser(String author, String authorKey, String hostID, String regPort) {
      ...
      stopPlaying = false;
      try{
        tcpUrl = "http://" + hostID + ":" + regPort + "/";
        System.out.println("Opening connection to: " + tcpUrl);
        mediaConnection = (MusicBrowserTcpProxy)new MusicBrowserTcpProxy(hostID, Integer.parseInt(regPort));
   ...
   public void rebuildTree(){
      ...
      try{
        ...
        if(inSearch){
          ...
          String[] ls = mediaConnection.getTitles();
          listSize = ls.length;
        }
        String[] musicList = mediaConnection.getTitles();
        ...
        if(inSearch){
          ...
          aMD = mediaConnection.findAlbum(musicList[i]);
        }
      ...
      try{
         ...
         if(node!=null){
            ...
            if(inSearch){
              ...
              md = mediaConnection.findAlbum(nodeLabel);
              tk = mediaConnection.findTrack(nodeLabel);
            }
            ...
            for(int i = 0; i < md.genre.size(); i++){
              genreJCB.addItem(md.genre.get(i));
            }
            //Album or Track spacific
   ...
   public void actionPerformed(ActionEvent e){
      ...
      if(e.getActionCommand().equals("Exit")) {
        ...
        if(inSearch){
          ...
          mediaConnection.removeTrack(trackJTF.getText().trim());
        }
        trackJTF.setText("");
        rankJTF.setText("");
        timeJTF.setText("");
        ...
        if(inSearch){
          ...
          mediaConnection.removeAlbum(albumJTF.getText().trim());
        }
        trackJTF.setText("");
        rankJTF.setText("");
        timeJTF.setText("");
        ...
        rebuildTree();
         ...
         try{
            ...
            if(node!=null){
               String nodeLabel = (String)node.getUserObject();
               MusicAlbum md = mediaConnection.findAlbum(nodeLabel);
